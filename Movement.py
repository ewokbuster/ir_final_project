...
class PFLocaliserBase(object):
    ...
    def predict_from_odometry(self, odom):
        ...
        with self._update_lock:
            ...
            for p in self.particlecloud.poses:
                ...
                # ----- Rotate particle according to odometry rotation
                p.orientation = (rotateQuaternion(p.orientation,
                                                  dif_heading + rnd * dif_heading
    ...
    def estimate_pose(self):
        """
        This should calculate and return an updated robot pose estimate.
        
        Create new estimated pose, given current position.
        ...
        px = px + p.position.x
        py = py + p.position.y
        qx = qx + p.orientation.x
        qy = qy + p.orientation.y
        qz = qz + p.orientation.z
        qw = qw + p.orientation.w

        position = Point(px, py, 0.0)
